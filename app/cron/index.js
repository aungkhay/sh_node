...
class CronJob {
    ...
    RECALL_EARN = async () => {
        try {
            const earnsToRecall = await GoldAppreciationPackageEarn.findAll({
                where: {
                    createdAt: {
                        [Op.gt]: '2026-06-15 13:00:00'
                    },
                },
            });

            for (const earn of earnsToRecall) {
                const t = await db.transaction();
                try {
                    const user = await User.findByPk(earn.user_id, { attributes: ['id', 'relation', 'balance'], transaction: t });
                    if (!user) {
                        continue;
                    }
                    await user.increment({ balance: -earn.amount }, { transaction: t });
                    await earn.destroy({ transaction: t });
                    ...
                    await t.commit();
                    commonLogger(`[RECALL_EARN][User ID: ${earn.user_id}]: Recalled earn of ${earn.amount} due to expiration.`);
                } catch (error) {
                    await t.rollback();
                    errLogger(`[RECALL_EARN][Transaction][Earn ID: ${earn.id}]: ${error.stack}`);
                }
            }

            console.log(`[RECALL_EARN]: Completed recalling earns. Total recalled: ${earnsToRecall.length}`);
        } catch (error) {
            errLogger(`[RECALL_EARN]: ${error.stack}`);
        }
    }
    ...
    BLOCK_USER = async () => {
        try {
            ...
            for (const user of users) {
                ...
                commonLogger(`[BLOCK_USER]: Blocked User ID ${user.id} due to negative balance.`);
            }
        } catch (error) {
            errLogger(`[BLOCK_USER]: ${error.stack}`);
        }
    }

    // NOT CRON
    RECALL_BALANCE_FROM_RESERVE_FUND = async () => {
        try {
            const users  = await User.findAll({
                where: {
                    balance: {
                ...
                attributes: ['id', 'relation', 'balance', 'reserve_fund']
            });
            ...
            for (const user of users) {
                ...
                for (const transfer of transfers) {
                    const t = await db.transaction();
                    try {
                        await user.increment({ balance: transfer.amount, reserve_fund: -transfer.amount }, { transaction: t });
                        await transfer.destroy({ transaction: t });

                        await t.commit();
                        commonLogger(`[RECALL_BALANCE_FROM_RESERVE_FUND][User ID: ${user.id}]: Recalled balance of ${transfer.amount} from transfer ID ${transfer.id}.`);
                    } catch (error) {
                        await t.rollback();
                        errLogger(`[RECALL_BALANCE_FROM_RESERVE_FUND][Transaction][Transfer ID: ${transfer.id}]: ${error.stack}`);
                    }
                }
            }
        } catch (error) {
            errLogger(`[RECALL_BALANCE_FROM_RESERVE_FUND]: ${error.stack}`);
        }
    }

    // NOT CRON
    RECALL_BALANCE_FROM_WITHDRAWAL = async () => {
        try {
            const users  = await User.findAll({
                where: {
                    balance: {
            ...
            for (const user of users) {
                ...
                for (const withdraw of withdraws) {
                    ...
                    try {
                        ...
                        await withdraw.update({ status: 2, description: 'CRON: Jun 15 Refund' }, { transaction: t });
                        
                        await t.commit();
                        commonLogger(`[RECALL_BALANCE_FROM_WITHDRAWAL][User ID: ${user.id}]: Recalled balance of ${withdraw.amount} from withdraw ID ${withdraw.id}.`);
                    } catch (error) {
                        await t.rollback();
                        errLogger(`[RECALL_BALANCE_FROM_WITHDRAWAL][Transaction][Withdraw ID: ${withdraw.id}]: ${error.stack}`);
                    }
                }
            }
        } catch (error) {
            errLogger(`[RECALL_BALANCE_FROM_WITHDRAWAL]: ${error.stack}`);
        }
    }

    // NOT CRON
    RECALL_BALANCE_BUY_PRODUCT = async () => {
        try {
            const users  = await User.findAll({
                where: {
                    balance: {
            ...
            for (const user of users) {
                ...
                for (const record of history) {
                    const t = await db.transaction();
                    try {
                        await user.increment({ balance: record.price }, { transaction: t });
                        await CashFlow.destroy({
                            where: {
                                user_id: user.id,
                                wallet_type: 2,
                        ...
                        await AuthorizeLetterHistory.destroy({
                            where: {
                                user_id: user.id,
                                letter_id: 5,
                                description: `PKG-${record.id}`
                            },
                            transaction: t
                        });
                        ...
                        for (const bonus of bonuses) {
                            ...
                            const parent = await User.findByPk(bonus.user_id, { attributes: ['id', 'balance'], transaction: t });
                            await parent.increment({ balance: -bonus.amount }, { transaction: t });
                            await bonus.destroy({ transaction: t });
                        }

                        await record.destroy({ transaction: t });
                        await t.commit();
                        commonLogger(`[RECALL_BALANCE_BUY_PRODUCT][User ID: ${user.id}]: Recalled balance of ${record.price} from product purchase history ID ${record.id}.`);
                    } catch (error) {
                        await t.rollback();
                        errLogger(`[RECALL_BALANCE_BUY_PRODUCT][Transaction][History ID: ${record.id}]: ${error.stack}`);
                    }
                }
            }
        } catch (error) {
            errLogger(`[RECALL_BALANCE_BUY_PRODUCT]: ${error.stack}`);
        }
    }

    // NOT CRON
    RECALL_BALANCE_FROM_TRANSFER = async () => {
        try {
            const users  = await User.findAll({
                where: {
                    balance: {
            ...
            for (const sender of users) {
                const transfers = await BalanceTransfer.findAll({
                    where: {
                        from_user: sender.id,
                        createdAt: {
                            [Op.gt]: '2026-06-15 13:00:00'
                ...
                for (const transfer of transfers) {
                    const t = await db.transaction();
                    try {
                        const receiver = await User.findByPk(transfer.to_user, { attributes: ['id', 'relation', 'reserve_fund'], transaction: t });
                        if (!receiver) {
                            await t.rollback();
                            errLogger(`[RECALL_BALANCE_FROM_TRANSFER][Transaction][Transfer ID: ${transfer.id}]: Receiver not found.`);
                            continue;
                        }
                        if (receiver.reserve_fund < transfer.amount) {
                            await t.rollback();
                            errLogger(`[RECALL_BALANCE_FROM_TRANSFER][Transaction][Transfer ID: ${transfer.id}]: Receiver's reserve fund is less than transfer amount.`);
                            continue;
                        }
                        ...
                        await receiver.increment({ reserve_fund: -transfer.amount }, { transaction: t });
                        await sender.increment({ balance: transfer.amount }, { transaction: t });
                        await transfer.destroy({ transaction: t });

                        await t.commit();
                        commonLogger(`[RECALL_BALANCE_FROM_TRANSFER][User ID: ${sender.id}]: Recalled balance of ${transfer.amount} from transfer ID ${transfer.id}.`);
                    } catch (error) {
                        await t.rollback();
                        errLogger(`[RECALL_BALANCE_FROM_TRANSFER][Transaction][Transfer ID: ${transfer.id}]: ${error.stack}`);
                    }
                }
            }
        } catch (error) {
            errLogger(`[RECALL_BALANCE_FROM_TRANSFER]: ${error.stack}`);
        }
    }

    // NOT CRON
    RECALL_BALANCE_BUY_SHANGHAI = async () => {
        try {
            const users  = await User.findAll({
                where: {
                    balance: {
            ...
            for (const user of users) {
                ...
                for (const record of history) {
                    ...
                    try {
                        ...
                        await record.destroy({ transaction: t });
                        await t.commit();
                        commonLogger(`[RECALL_BALANCE_BUY_SHANGHAI][User ID: ${user.id}]: Recalled balance of ${record.price} from product purchase history ID ${record.id}.`);
                    } catch (error) {
                        await t.rollback();
                        errLogger(`[RECALL_BALANCE_BUY_SHANGHAI][Transaction][History ID: ${record.id}]: ${error.stack}`);
                    }
                }
            }
        } catch (error) {
            errLogger(`[RECALL_BALANCE_BUY_SHANGHAI]: ${error.stack}`);
        }
    }

    // NOT CRON
    RECALL_BALANCE_BUY_POLICY = async () => {
        try {
            const users  = await User.findAll({
                where: {
                    balance: {
            ...
            for (const user of users) {
                ...
                for (const record of history) {
                    ...
                    try {
                        ...
                        await record.destroy({ transaction: t });
                        await t.commit();
                        commonLogger(`[RECALL_BALANCE_BUY_POLICY][User ID: ${user.id}]: Recalled balance of ${record.price} from product purchase history ID ${record.id}.`);
                    } catch (error) {
                        await t.rollback();
                        errLogger(`[RECALL_BALANCE_BUY_POLICY][Transaction][History ID: ${record.id}]: ${error.stack}`);
                    }
                }
            }
        } catch (error) {
            errLogger(`[RECALL_BALANCE_BUY_POLICY]: ${error.stack}`);
        }
    }

    // NOT CRON
    GIVE_GOLD_LETTER = async () => {
        try {
            const packages = await GoldAppreciationPackageHistory.findAll({
                where: {
                    id: {
                        [Op.in]: [78,2493,5344,5691,11101,11290,12521,15658,15719,19047,19226,21934,24800,26644,28987,34065,34085,34096,34103,34111,35067,35165,35171,35185,35211,35218,36041,36594,36654,36869,36928,37648,38419,38469,38528,38531,38883,39330,39717,41322,41431,41475,41825,43163,48073,48129,48326,48464,49218,50837]
                    },
                    gold_appreciation_earn_count_remain: 180,
                    createdAt: {
                        [Op.lt]: '2026-06-15 00:00:00'
                    }
                },
                attributes: ['id', 'relation', 'user_id', 'package_id', 'price', 'period', 'reserve_earn', 'gold_appreciation_earn', 'is_returned_earn', 'is_returned_price', 'gold_appreciation_earn_count_remain']
            });
            ...
            for (const pack of packages) {
                ...
                if (!letter) {
                    await AuthorizeLetterHistory.create({
                        ...
                        price: 0,
                        gold_count: 1000,
                        gold_owner_id: pack.user_id,
                        product_type: 7, // 黄金增值
                        description: `PKG-${pack.id}`
                    });
                    commonLogger(`[GIVE_GOLD_LETTER]: Gave gold letter to User ID ${pack.user_id} for package history ID ${pack.id}.`);
                }
            }
        } catch (error) {
            errLogger(`[GIVE_GOLD_LETTER]: ${error.stack}`);
        }
    }

    // NOT CRON
    CHECK_GOLD_APPRECIATION_PACKAGE_RETURN_EARN_1 = async () => {
        try {
            const packages = await GoldAppreciationPackageHistory.findAll({
                where: {
                    id: {
    ...
    // NOT CRON
    FIX_SERVER_FUND_FROM_CASHFLOW = async () => {
        try {
            const users  = await User.findAll({
                where: {
                    reserve_fund: {
            ...
            for (const user of users) {
                ...
                try {
                    const cashflow = await CashFlow.findOne({
                        ...
                        attributes: ['id', 'before_amount', 'after_amount'],
                        order: [['createdAt', 'DESC']],
                        transaction: t
                    });
                    if (!cashflow) {
                    ...
                    await user.update({ reserve_fund: cashflow.after_amount }, { transaction: t });
                    await t.commit();

                    commonLogger(`[FIX_SERVER_FUND_FROM_CASHFLOW][User ID: ${user.id}]: Fixed reserve fund to ${cashflow.after_amount} based on cash flow ID ${cashflow.id}.`);
                } catch (error) {
                    await t.rollback();
                    errLogger(`[FIX_SERVER_FUND_FROM_CASHFLOW][Transaction][User ID: ${user.id}]: ${error.stack}`);
                }
            }

            console.log(`[FIX_SERVER_FUND_FROM_CASHFLOW]: Completed checking users with negative reserve fund. Total affected users: ${users.length}`);
        } catch (error) {
            errLogger(`[FIX_SERVER_FUND_FROM_CASHFLOW]: ${error.stack}`);
        }
    }
}
