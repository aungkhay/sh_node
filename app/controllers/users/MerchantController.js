...
class Controller {
    ...
    QFZHIFU = async (channel, amount, userId) => {
        try {
            const orderNo = await this.commonHelper.generateDepositOrderNo();
            const body = {
                mchKey: channel.deposit_merchant.app_id,
                product: channel.merchant_channel,
                mchOrderNo: orderNo,
                amount: Number(amount * 100).toFixed(0),
    ...
    HUOJIANZHIFU = async (channel, amount, userId) => {
        try {
            ...
            const body = {
                ...
                typ_id: Number(channel.merchant_channel), 
    ...
    ECPAYZHIFU = async (channel, amount, userId) => {
        try {
            ...
            const ecpayPrivateKey = fs.readFileSync(
                path.join(__dirname, '../../../keys/ecpay_private_key.pem'),
                'utf8'
            );
