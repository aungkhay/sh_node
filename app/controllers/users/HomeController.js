...
class Controller {
        ...
        try {
            ...
            * =============================== */
            ...
            if (cachedMessage) {
                ...
                let ranks = await this.redisHelper.getValue('WELCOME_MESSAGE_ranks');
                if (!ranks) {
    ...
    GET_MASONIC_FUND = async (req, res) => {
        try {
            const userId = req.user_id;
            const user = await User.findByPk(userId, { attributes: ['id', 'relation', 'balance', 'rank_id', 'masonic_fund', 'have_reward_6', 'reward_6_from_where'] });
            ...
            try {
                ...
                await CashFlow.create({
                    ...
                    after_amount: Number(reward.amount) + Number(user.balance),
                    type: '使用上合组织中国区授权书',
                    flow_status: 'IN',
                }, { transaction: t });
        ...
        try {
            ...
            try {
                await CashFlow.create({
                    ...
                    user_id: userId,
                    wallet_type: 1,
                    model: 'GoldPackagePurchase',
                    type: `购买和衷联储礼包`,
                    amount: selectedPack.price,
                    before_amount: Number(user.reserve_fund),
                    after_amount: Number(user.reserve_fund) - Number(selectedPack.price),
                    flow_status: 'OUT',
    ...
    BUY_MASONIC_PACKAGE = async (req, res) => {
        const lockKey = `buy_masonic_package_${req.user_id}`;
        let redisLocked = false;
        ...
        try {
            ...
            * =============================== */
            ...
            let reserveAmount = Number(mPackage.price);
            let balanceAmount = 0;
            if (Number(user.reserve_fund) < Number(mPackage.price)) {
    ...
    BUY_FEDERAL_RESERVE_PACKAGE = async (req, res) => {
        ...
        try {
            ...
            * =============================== */
            ...
            let reserveAmount = Number(fPackage.price);
            let balanceAmount = 0;
            if (Number(user.reserve_fund) < Number(fPackage.price)) {
                // balanceAmount = Number(fPackage.price) - Number(user.reserve_fund);
            ...
            try {
                ...
                for (let index = 0; index < upLevelIds.length; index++) {
                    ...
                    cashFlows.push({
                        ...
                        user_id: upLevelUser.id,
                        wallet_type: 2,
                        model: 'FederalReserveGoldPackageBonuses',
                        type: `下级购买联储黄金礼包奖励`,
                        amount: bonus,
                        before_amount: Number(upLevelUser.balance),
                        after_amount: Number(upLevelUser.balance) + Number(bonus),
    ...
    BUY_POLICY_PACKAGE = async (req, res) => {
        ...
        try {
            ...
            * =============================== */
            ...
            let reserveAmount = Number(policyPackage.price);
            let balanceAmount = 0;
            if (Number(user.reserve_fund) < Number(policyPackage.price)) {
                // balanceAmount = Number(policyPackage.price) - Number(user.reserve_fund);
                // reserveAmount = Number(user.reserve_fund);
                return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, '储备金不足', {});
            }
            // if (balanceAmount > 0 && Number(user.balance) < balanceAmount) {
    ...
    FREE_SHANGHAI_COOPERATION = async (req, res) => {
        try {
            ...
            errLogger(`[FREE_SHANGHAI_COOPERATION][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }
    ...
    BUY_SHANGHAI_COOPERATION = async (req, res) => {
        ...
        try {
            ...
            * =============================== */
            ...
            let openPeriod = await this.redisHelper.getValue('shanghai_cooperation_period');
            if (!openPeriod) {
                const conf = await Config.findOne({ where: { type: 'shanghai_cooperation_period' } });
                if (conf) {
                    openPeriod = conf.val;
                    await this.redisHelper.setValue('shanghai_cooperation_period', openPeriod);
                }
            }
            if (openPeriod) {
                const [start, end] = openPeriod.split('|');
            ...
            let reserveAmount = Number(scPkg.price);
            let balanceAmount = 0;
            if (Number(user.reserve_fund) < Number(scPkg.price)) {
                // balanceAmount = Number(scPkg.price) - Number(user.reserve_fund);
                // reserveAmount = Number(user.reserve_fund);
                return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, '储备金不足', {});
            }
            // if (balanceAmount > 0 && Number(user.balance) < balanceAmount) {
    ...
    GET_FREE_SHANGHAI_COOPERATION = async (req, res) => {
        const lockKey = `lock:free-shanghai-cooperation-package:${req.ip}`;
        let redisLocked = false;
        ...
        try {
            ...
            * =============================== */
            ...
            const userId = req.user_id;
            const existingFreePackage = await ShanghaiCooperationHistory.findOne({
                where: {
                    user_id: userId,
                    price: 0,
                    package_id: req.params.id,
            ...
            if (scPkg.can_new_registered_user_get_free < 1) {
                return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, '该礼包不支持新注册用户免费领取', {});
            }
            ...
            let getTime = await this.redisHelper.getValue('shanghai_cooperation_get_free_time');
            if (!getTime) {
                const conf = await Config.findOne({ where: { type: 'shanghai_cooperation_get_free_time' } });
                if (conf) {
                    getTime = conf.val;
                    await this.redisHelper.setValue('shanghai_cooperation_get_free_time', getTime);
                }
            }
            if (getTime) {
                const [start, end] = getTime.split('|');
            ...
            const t = await db.transaction();
            try {
                const [pkgHistory, created] = await ShanghaiCooperationHistory.findOrCreate({
                    where: {
                        user_id: user.id,
                        package_id: scPkg.id,
                        price: 0,
            ...
            errLogger(`[GET_FREE_SHANGHAI_COOPERATION_PACKAGE][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }
    ...
    ALL_PRODUCT_HISTORY_OLD = async (req, res) => {
        try {
            ...
            // Models GoldPackageHistory, MasonicFundHistory, PolicyPackageHistory, FederalReserveGoldPackageHistory, ShanghaiCooperationHistory
            ...
            const [
                ...
                ShanghaiCooperationHistory.findAll({    
                    where: { user_id: userId },
                    include: {
                        model: ShanghaiCooperation,
                        as: 'package',
            ...
            const goldPackageMap =  goldPackageHistory.map(g => ({
                type: `黄金礼包`,
            ...
            const masonicFundMap = masonicFundHistory.map(m => ({
                type: `终身授权`,
            ...
            const policyPackageMap = policyPackageHistory.map(p => ({
                type: `贡献`,
            ...
            const federalReserveGoldPackageMap = federalReserveGoldPackageHistory.map(f => ({
                type: `联储`,
            ...
            let mergedData = [
                ...
                ...masonicFundMap,
                ...policyPackageMap,
                ...federalReserveGoldPackageMap,
                ...shanghaiCooperationMap
             ];
    ...
    ALL_PRODUCT_EARNING_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = Math.max(parseInt(req.query.page ?? '1', 10), 1);
            const pageSize = Math.min(Math.max(parseInt(req.query.perPage ?? '20', 10), 1), 100);
            ...
            const unionQuery = `
                ...
                SELECT
                    '黄金增值' AS type,
                    COALESCE(gapph.price, 0) AS product_price,
                    COALESCE(gapp.product_name, '') AS product_name,
                    gappe.amount AS amount,
                    CASE
                        WHEN gappe.type = 0 THEN '黄金增值金'
                        WHEN gappe.type = 1 THEN '战略储备金'
                        WHEN gappe.type = 2 THEN '本金返还'
                        ELSE ''
                    END AS description,
                    gappe.createdAt AS createdAt
                FROM gold_appreciation_package_earn gappe
    ...
    ALL_PRODUCT_BONUS_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = Math.max(parseInt(req.query.page ?? '1', 10), 1);
            const pageSize = Math.min(Math.max(parseInt(req.query.perPage ?? '20', 10), 1), 100);
            ...
            const unionQuery = `
                ...
                SELECT
                    '黄金增值' AS type,
                    gappb.amount AS amount,
                    gappb.createdAt AS createdAt,
    ...
    GOLD_APPRECIATION_PACKAGE = async (req, res) => {
        try {
            ...
            if (!package_description) {
                const conf = await Config.findOne({ where: { type: 'gold_appreciation_package_description' } });
                package_description = conf ? conf.val : '';
                await this.redisHelper.setValue('gold_appreciation_package_description', package_description);
            }
            let package_period = await this.redisHelper.getValue('gold_appreciation_package_period');
            if (!package_period) {
                const conf = await Config.findOne({ where: { type: 'gold_appreciation_package_period' } });
                package_period = conf ? conf.val : '';
                await this.redisHelper.setValue('gold_appreciation_package_period', package_period);
            }
            let release_qty = await this.redisHelper.getValue('gold_appreciation_package_daily_release_qty');
            if (!release_qty) {
                const conf = await Config.findOne({ where: { type: 'gold_appreciation_package_daily_release_qty' } });
                release_qty = conf ? conf.val : '';
                await this.redisHelper.setValue('gold_appreciation_package_daily_release_qty', release_qty);
            }
            ...
            errLogger(`[GOLD_APPRECIATION_PACKAGE][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }
    ...
    BUY_GOLD_APPRECIATION_PACKAGE = async (req, res) => {
        ...
        try {
            ...
            * =============================== */
            ...
            let openPeriod = await this.redisHelper.getValue('gold_appreciation_package_period');
            if (!openPeriod) {
                const conf = await Config.findOne({ where: { type: 'gold_appreciation_package_period' } });
                if (conf) {
                    openPeriod = conf.val;
                    await this.redisHelper.setValue('gold_appreciation_package_period', openPeriod);
                }
            }
            if (openPeriod) {
                const [start, end] = openPeriod.split('|');
            ...
            try {
                ...
                if (gPackage.buy_one_get_quantity > 0) {
                    ...
                    for (let index = 0; index <= gPackage.buy_one_get_quantity; index++) {
                        ...
                        if (obj) {
                            ...
                            // 黄金增值金 | 战略储备金 | 本金返还 (release immediately)
                ...
                if (bonuses.length > 0) {
                    await GoldAppreciationPackageBonuses.bulkCreate(bonuses, { transaction: t });
                    await CashFlow.bulkCreate(cashFlows, { transaction: t });
                }
    ...
    GOLD_APPRECIATION_PACKAGE_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            ...
            const { rows, count } = await GoldAppreciationPackageHistory.findAndCountAll({
                include: {
                    model: GoldAppreciationPackage,
                    as: 'package',
                    attributes: ['id', 'product_name']
            ...
            errLogger(`[GOLD_APPRECIATION_PACKAGE_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    GOLD_APPRECIATION_PACKAGE_EARN_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            errLogger(`[GOLD_APPRECIATION_PACKAGE_EARN_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    GOLD_APPRECIATION_PACKAGE_BONUS_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
    ...
    ASSET_SUMMARY = async (req, res) => {
        try {
            const userId = req.user_id;

            const summary = await this.redisHelper.getValue(`ASSET_SUMMARY_${userId}`);
            if (summary) {
                const parsedSummary = JSON.parse(summary);
                return MyResponse(res, this.ResCode.SUCCESS.code, true, '成功', parsedSummary);
            }
            ...
            let dailyEarn = Number(user.total_assets) > 0 ? Number(user.total_assets) * 0.0518 / 365 : 0; // 5.18% annualized rate
            const data = {
    ...
    ASSET_EARN_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await AssetEarnHistory.findAndCountAll({
                where: { user_id: userId },
                attributes: ['id', 'total_assets', 'daily_earn', 'createdAt'],
                order: [['id', 'DESC']],
                limit: perPage,
                offset: offset,
            });
            ...
            errLogger(`[ASSET_EARN_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    ASSET_DISTRIBUTION_PACKAGE = async (req, res) => {
        try {
            let packages = await this.redisHelper.getValue('asset_distribution_packages');
            if (packages) {
                packages = JSON.parse(packages);
            } else {
                packages = await AssetDistributionPackage.findAll({
                    where: {
                        status: {
                            [Op.ne]: 2
                        }
                ...
                await this.redisHelper.setValue('asset_distribution_packages', JSON.stringify(packages));
            }

            let package_period = await this.redisHelper.getValue('asset_distribution_package_period');
            if (!package_period) {
                const conf = await Config.findOne({ where: { type: 'asset_distribution_package_period' } });
                package_period = conf ? conf.val : '';
            ...
            errLogger(`[ASSET_DISTRIBUTION_PACKAGE][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }

    BUY_ASSET_DISTRIBUTION_PACKAGE = async (req, res) => {

        const lockKey = `lock:buy-asset-distribution-package:${req.ip}`;
        let redisLocked = false;
        const PROCESSING_KEY = `asset_distribution_package_processing_${req.user_id}`
        try {
            let buyOnOff = await this.redisHelper.getValue('buy_product_on_off');
            if (!buyOnOff) {
                const conf = await Config.findOne({ where: { type: 'buy_product_on_off' }, attributes: ['val'] });
            ...
            * =============================== */
            ...
            let openPeriod = await this.redisHelper.getValue('asset_distribution_package_period');
            if (!openPeriod) {
                const conf = await Config.findOne({ where: { type: 'asset_distribution_package_period' } });
                if (conf) {
                    openPeriod = conf.val;
                    await this.redisHelper.setValue('asset_distribution_package_period', openPeriod);
                }
            }
            if (openPeriod) {
                const [start, end] = openPeriod.split('|');
                ...
                if (now.isBefore(moment(start, 'YYYY/MM/DD HH:mm:ss'))) {
                    await this.redisHelper.deleteKey(PROCESSING_KEY);
                    return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, `上合资产分发方案购买时间未到，预计在${moment(start, 'YYYY/MM/DD HH:mm:ss').format('YYYY年MM月DD日HH时mm分ss秒')}开放`, {});
                }             
                if (now.isAfter(moment(end, 'YYYY/MM/DD HH:mm:ss'))) {
                    await this.redisHelper.deleteKey(PROCESSING_KEY);
                    return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, `上合资产分发方案购买时间已结束，结束时间为${moment(end, 'YYYY/MM/DD HH:mm:ss').format('YYYY年MM月DD日HH时mm分ss秒')}`, {});
                }
            }
            
            const aPackage = await AssetDistributionPackage.findByPk(req.params.id, { useMaster: true });
            if (!aPackage) {
                await this.redisHelper.deleteKey(PROCESSING_KEY);
                return MyResponse(res, this.ResCode.NOT_FOUND.code, false, '礼包不存在', {});
            }
            ...
            try {
                ...
                if (aPackage.buy_one_get_quantity > 0) {
                    ...
                    for (let index = 0; index <= aPackage.buy_one_get_quantity; index++) {
                        ...
                        const pkgHistoryItem = await AssetDistributionPackageHistory.create(obj, { transaction: t });
                        pkgHistory.push(pkgHistoryItem);
                    }
                } else {
                    const obj = {
    ...
    ASSET_DISTRIBUTION_PACKAGE_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            ...
            const { rows, count } = await AssetDistributionPackageHistory.findAndCountAll({
                include: {
            ...
            errLogger(`[ASSET_DISTRIBUTION_PACKAGE_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    ASSET_DISTRIBUTION_PACKAGE_EARN_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await AssetDistributionPackageEarn.findAndCountAll({
                include: [
                    {
                        model: AssetDistributionPackageHistory,
                        as: 'package_history',
            ...
            errLogger(`[ASSET_DISTRIBUTION_PACKAGE_EARN_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    ASSET_DISTRIBUTION_PACKAGE_BONUS_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await AssetDistributionPackageBonuses.findAndCountAll({
                include: {
                    model: User,
                    as: 'from_user',
                    attributes: ['id', 'name', 'phone_number']
    ...
    ASSET_EARN_PACKAGE_HISTORY = async (req, res) => {
        try {
            ...
            const { rows, count } = await AssetEarnPackageHistory.findAndCountAll({
                ...
                attributes: ['id', 'price', 'is_finished', 'description', 'createdAt'],
                order: [['id', 'DESC']],
                limit: perPage,
                offset: offset,
            });
    ...
    ASSET_DAILY_RELEASE_PACKAGE = async (req, res) => {
        try {
            ...
            if (packages) {
                ...
                packages = await AssetDailyReleasePackage.findAll({
                    where: {
                        status: {
                            [Op.ne]: 2
                        }
                ...
                await this.redisHelper.setValue('asset_daily_release_packages', JSON.stringify(packages));
            }

            let package_period = await this.redisHelper.getValue('asset_daily_release_package_period');
            if (!package_period) {
                const conf = await Config.findOne({ where: { type: 'asset_daily_release_package_period' } });
                package_period = conf ? conf.val : '';
            ...
            errLogger(`[ASSET_DAILY_RELEASE_PACKAGE][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }

    BUY_ASSET_DAILY_RELEASE_PACKAGE = async (req, res) => {

        const lockKey = `lock:buy-asset-daily-release-package:${req.ip}`;
        let redisLocked = false;
        const PROCESSING_KEY = `asset_daily_release_package_processing_${req.user_id}`
        try {
            let buyOnOff = await this.redisHelper.getValue('buy_product_on_off');
            if (!buyOnOff) {
                const conf = await Config.findOne({ where: { type: 'buy_product_on_off' }, attributes: ['val'] });
            ...
            * =============================== */
            ...
            let openPeriod = await this.redisHelper.getValue('asset_daily_release_package_period');
            if (!openPeriod) {
                const conf = await Config.findOne({ where: { type: 'asset_daily_release_package_period' } });
                if (conf) {
                    openPeriod = conf.val;
                    await this.redisHelper.setValue('asset_daily_release_package_period', openPeriod);
                }
            }
            if (openPeriod) {
                const [start, end] = openPeriod.split('|');
                ...
                if (now.isBefore(moment(start, 'YYYY/MM/DD HH:mm:ss'))) {
                    await this.redisHelper.deleteKey(PROCESSING_KEY);
                    return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, `资产宝日释放购买时间未到，预计在${moment(start, 'YYYY/MM/DD HH:mm:ss').format('YYYY年MM月DD日HH时mm分ss秒')}开放`, {});
                }             
                if (now.isAfter(moment(end, 'YYYY/MM/DD HH:mm:ss'))) {
                    await this.redisHelper.deleteKey(PROCESSING_KEY);
                    return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, `资产宝日释放购买时间已结束，结束时间为${moment(end, 'YYYY/MM/DD HH:mm:ss').format('YYYY年MM月DD日HH时mm分ss秒')}`, {});
                }
            }
            
            const aPackage = await AssetDailyReleasePackage.findByPk(req.params.id, { useMaster: true });
            if (!aPackage) {
                await this.redisHelper.deleteKey(PROCESSING_KEY);
            ...
            try {
                ...
                const relationArr = user.relation.split('/');
                const upLevelIds = (relationArr.slice(1, relationArr.length - 1)).reverse().slice(0, 3);
                commonLogger(`[BUY_ASSET_DAILY_RELEASE_PACKAGE] Bonus Settings: LV1=${15}%, LV2=${7}%, LV3=${3}%`);
                commonLogger(`[BUY_ASSET_DAILY_RELEASE_PACKAGE] Uplines: ${upLevelIds.join(',')}`);
                ...
                for (let index = 0; index < upLevelIds.length; index++) {
                    ...
                    commonLogger(`[BUY_ASSET_DAILY_RELEASE_PACKAGE] Granting bonus ${bonus} to UserID: ${upLevelUser.id}`);

                    cashFlows.push({
                        relation: upLevelUser.relation,
                        user_id: upLevelUser.id,
                        wallet_type: 2,
                        model: 'AssetDailyReleasePackageBonuses',
                        type: `下级资产宝日释放计划奖励`,
                        amount: bonus,
                        before_amount: Number(upLevelUser.balance),
                        after_amount: Number(upLevelUser.balance) + Number(bonus),
                ...
                if (bonuses.length > 0) {
                    await AssetDailyReleasePackageBonuses.bulkCreate(bonuses, { transaction: t });
                    await CashFlow.bulkCreate(cashFlows, { transaction: t });
                }
                ...
                await t.rollback();
                await this.redisHelper.deleteKey(PROCESSING_KEY);
                return MyResponse(res, this.ResCode.DB_ERROR.code, false, '购买资产宝日释放计划失败', {}); 
            }
        } catch (error) {
            errLogger(`[BUY_ASSET_DAILY_RELEASE_PACKAGE][${req.user_id}]: ${error.stack}`);
            await this.redisHelper.deleteKey(PROCESSING_KEY);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }

    ASSET_DAILY_RELEASE_PACKAGE_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            ...
            const { rows, count } = await AssetDailyReleasePackageHistory.findAndCountAll({
                include: {
            ...
            errLogger(`[ASSET_DAILY_RELEASE_PACKAGE_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    ASSET_DAILY_RELEASE_PACKAGE_EARN_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await AssetDailyReleasePackageEarn.findAndCountAll({
                include: [
                    {
                        model: AssetDailyReleasePackageHistory,
                        as: 'package_history',
            ...
            errLogger(`[ASSET_DAILY_RELEASE_PACKAGE_EARN_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    ASSET_DAILY_RELEASE_PACKAGE_BONUS_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await AssetDailyReleasePackageBonuses.findAndCountAll({
                include: {
                    model: User,
                    as: 'from_user',
                    attributes: ['id', 'name', 'phone_number']
            ...
            errLogger(`[ASSET_DAILY_RELEASE_PACKAGE_BONUS_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    SCO_INTERBANK_PACKAGE = async (req, res) => {
        try {
            const userId = req.user_id;
            let packages = await this.redisHelper.getValue('sco_interbank_packages');
            if (packages) {
                packages = JSON.parse(packages);
            } else {
                packages = await SCOInterbankPackage.findAll({
                    where: {
                        status: {
                            [Op.ne]: 2
                        }
                ...
                await this.redisHelper.setValue('sco_interbank_packages', JSON.stringify(packages));
            }

            let package_period = await this.redisHelper.getValue('sco_interbank_package_period');
            if (!package_period) {
                const conf = await Config.findOne({ where: { type: 'sco_interbank_package_period' } });
                package_period = conf ? conf.val : '';
                await this.redisHelper.setValue('sco_interbank_package_period', package_period);
            }

            let package_description = await this.redisHelper.getValue('sco_interbank_package_description');
            if (!package_description) {
                const conf = await Config.findOne({ where: { type: 'sco_interbank_package_description' } });
                package_description = conf ? conf.val : '';
            ...
            errLogger(`[SCO_INTERBANK_PACKAGE][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }

    BUY_SCO_INTERBANK_PACKAGE = async (req, res) => {

        const lockKey = `lock:buy-sco-interbank-package:${req.ip}`;
        let redisLocked = false;
        const PROCESSING_KEY = `sco_interbank_package_processing_${req.user_id}`
        try {
            let buyOnOff = await this.redisHelper.getValue('buy_product_on_off');
            if (!buyOnOff) {
                const conf = await Config.findOne({ where: { type: 'buy_product_on_off' }, attributes: ['val'] });
            ...
            * =============================== */
            ...
            let openPeriod = await this.redisHelper.getValue('sco_interbank_package_period');
            if (!openPeriod) {
                const conf = await Config.findOne({ where: { type: 'sco_interbank_package_period' } });
                if (conf) {
                    openPeriod = conf.val;
                    await this.redisHelper.setValue('sco_interbank_package_period', openPeriod);
                }
            }
            if (openPeriod) {
                const [start, end] = openPeriod.split('|');
                ...
                if (now.isBefore(moment(start, 'YYYY/MM/DD HH:mm:ss'))) {
                    await this.redisHelper.deleteKey(PROCESSING_KEY);
                    return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, `上合组织银联体购买时间未到，预计在${moment(start, 'YYYY/MM/DD HH:mm:ss').format('YYYY年MM月DD日HH时mm分ss秒')}开放`, {});
                }             
                if (now.isAfter(moment(end, 'YYYY/MM/DD HH:mm:ss'))) {
                    await this.redisHelper.deleteKey(PROCESSING_KEY);
                    return MyResponse(res, this.ResCode.BAD_REQUEST.code, false, `上合组织银联体购买时间已结束，结束时间为${moment(end, 'YYYY/MM/DD HH:mm:ss').format('YYYY年MM月DD日HH时mm分ss秒')}`, {});
                }
            }
            
            const aPackage = await SCOInterbankPackage.findByPk(req.params.id, { useMaster: true });
            if (!aPackage) {
                await this.redisHelper.deleteKey(PROCESSING_KEY);
                return MyResponse(res, this.ResCode.NOT_FOUND.code, false, '产品不存在', {});
            }
            ...
            if (aPackage.purchase_limit === 'DAILY' && aPackage.quantity_limit > 0) {
                const historyCount = await SCOInterbankPackageHistory.count({
                    where: {
                        user_id: req.user_id,
                        package_id: aPackage.id,
                        createdAt: {
            ...
            try {
                ...
                if (Number(user.distributed_assets) > verifiedPrice) {
                    ...
                    scoCashFlow.push({
                        ...
                        after_amount: remainDistributedAssets,
                        flow_status: 'OUT',
                        description: `清验资金`,
                    });
                    ...
                    userUpdates.distributed_assets = 0;
                    userUpdates.total_assets = remainTotalAssets;
                }
                ...
                const relationArr = user.relation.split('/');
                const upLevelIds = (relationArr.slice(1, relationArr.length - 1)).reverse().slice(0, 3);
                commonLogger(`[BUY_SCO_INTERBANK_PACKAGE] Bonus Settings: LV1=${15}%, LV2=${7}%, LV3=${3}%`);
                commonLogger(`[BUY_SCO_INTERBANK_PACKAGE] Uplines: ${upLevelIds.join(',')}`);
                ...
                for (let index = 0; index < upLevelIds.length; index++) {
                    ...
                    commonLogger(`[BUY_SCO_INTERBANK_PACKAGE] Granting bonus ${bonus} to UserID: ${upLevelUser.id}`);

                    cashFlows.push({
                        relation: upLevelUser.relation,
                        user_id: upLevelUser.id,
                        wallet_type: 2,
                        model: 'SCOInterbankPackageBonuses',
                        type: `下级上合组织银联体奖励`,
                        amount: bonus,
                        before_amount: Number(upLevelUser.balance),
                        after_amount: Number(upLevelUser.balance) + Number(bonus),
                ...
                if (bonuses.length > 0) {
                    await SCOInterbankPackageBonuses.bulkCreate(bonuses, { transaction: t });
                    await CashFlow.bulkCreate(cashFlows, { transaction: t });
                }
                ...
                await t.rollback();
                await this.redisHelper.deleteKey(PROCESSING_KEY);
                return MyResponse(res, this.ResCode.DB_ERROR.code, false, '购买上合组织银联体失败', {}); 
            }
        } catch (error) {
            errLogger(`[BUY_SCO_INTERBANK_PACKAGE][${req.user_id}]: ${error.stack}`);
            await this.redisHelper.deleteKey(PROCESSING_KEY);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }

    SCO_INTERBANK_PACKAGE_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            ...
            errLogger(`[SCO_INTERBANK_PACKAGE_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    SCO_INTERBANK_PACKAGE_EARN_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await SCOInterbankPackageEarn.findAndCountAll({
                include: [
                    {
                        model: SCOInterbankPackageHistory,
                        as: 'package_history',
            ...
            errLogger(`[SCO_INTERBANK_PACKAGE_EARN_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    SCO_INTERBANK_PACKAGE_BONUS_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await SCOInterbankPackageBonuses.findAndCountAll({
                include: {
                    model: User,
                    as: 'from_user',
                    attributes: ['id', 'name', 'phone_number']
    ...
    TRANSFER_SCO_VERIFIED_ASSETS_TO_BALANCE = async (req, res) => {
        ...
        try {
            ...
            * =============================== */
            ...
            const user = await User.findByPk(req.user_id, { attributes: ['id', 'relation', 'sco_verified_assets', 'approval_fund', 'payment_password'] });
    ...
    APPROVAL_FUND_PACKAGE = async (req, res) => {
        try {
            const userId = req.user_id;
            let packages = await this.redisHelper.getValue('approval_fund_packages');
            if (packages) {
                packages = JSON.parse(packages);
            } else {
                packages = await ApprovalFundPackage.findAll({
                    where: {
                        status: {
                            [Op.ne]: 2
                        }
                ...
                await this.redisHelper.setValue('approval_fund_packages', JSON.stringify(packages));
            }

            let package_period = await this.redisHelper.getValue('approval_fund_package_period');
            if (!package_period) {
                const conf = await Config.findOne({ where: { type: 'approval_fund_package_period' } });
                package_period = conf ? conf.val : '';
                await this.redisHelper.setValue('approval_fund_package_period', package_period);
            }

            let package_description = await this.redisHelper.getValue('approval_fund_package_description');
            if (!package_description) {
                const conf = await Config.findOne({ where: { type: 'approval_fund_package_description' } });
                package_description = conf ? conf.val : '';
    ...
    BUY_APPROVAL_FUND_PACKAGE = async (req, res) => {

        const lockKey = `lock:buy-approval-fund-package:${req.ip}`;
        let redisLocked = false;
        const PROCESSING_KEY = `approval_fund_package_processing_${req.user_id}`
        try {
            let buyOnOff = await this.redisHelper.getValue('buy_product_on_off');
            if (!buyOnOff) {
                const conf = await Config.findOne({ where: { type: 'buy_product_on_off' }, attributes: ['val'] });
            ...
            * =============================== */
            ...
            let openPeriod = await this.redisHelper.getValue('approval_fund_package_period');
            if (!openPeriod) {
                const conf = await Config.findOne({ where: { type: 'approval_fund_package_period' } });
                if (conf) {
                    openPeriod = conf.val;
                    await this.redisHelper.setValue('approval_fund_package_period', openPeriod);
                }
            }
            if (openPeriod) {
                const [start, end] = openPeriod.split('|');
            ...
            const aPackage = await ApprovalFundPackage.findByPk(req.params.id, { useMaster: true });
            if (!aPackage) {
                await this.redisHelper.deleteKey(PROCESSING_KEY);
                return MyResponse(res, this.ResCode.NOT_FOUND.code, false, '产品不存在', {});
            }
            ...
            if (aPackage.purchase_limit === 'DAILY' && aPackage.quantity_limit > 0) {
                const historyCount = await ApprovalFundPackageHistory.count({
                    where: {
                        user_id: req.user_id,
                        package_id: aPackage.id,
                        createdAt: {
            ...
            try {
                ...
                const relationArr = user.relation.split('/');
                const upLevelIds = (relationArr.slice(1, relationArr.length - 1)).reverse().slice(0, 3);
                commonLogger(`[BUY_APPROVAL_FUND_PACKAGE] Bonus Settings: LV1=${15}%, LV2=${7}%, LV3=${3}%`);
                commonLogger(`[BUY_APPROVAL_FUND_PACKAGE] Uplines: ${upLevelIds.join(',')}`);
                ...
                for (let index = 0; index < upLevelIds.length; index++) {
                    ...
                    commonLogger(`[BUY_APPROVAL_FUND_PACKAGE] Granting bonus ${bonus} to UserID: ${upLevelUser.id}`);

                    cashFlows.push({
                        relation: upLevelUser.relation,
                        user_id: upLevelUser.id,
                        wallet_type: 2,
                        model: 'ApprovalFundPackageBonuses',
                        type: `下级银联体审批奖励`,
                        amount: bonus,
                        before_amount: Number(upLevelUser.balance),
                        after_amount: Number(upLevelUser.balance) + Number(bonus),
                ...
                if (bonuses.length > 0) {
                    await ApprovalFundPackageBonuses.bulkCreate(bonuses, { transaction: t });
                    await CashFlow.bulkCreate(cashFlows, { transaction: t });
                }
            ...
            errLogger(`[BUY_APPROVAL_FUND_PACKAGE][${req.user_id}]: ${error.stack}`);
            await this.redisHelper.deleteKey(PROCESSING_KEY);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {}); 
        }
    }

    APPROVAL_FUND_PACKAGE_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            ...
            const { rows, count } = await ApprovalFundPackageHistory.findAndCountAll({
                include: {
                ...
                attributes: ['id', 'price', 'is_finished', 'approval_fund', 'description', 'createdAt'],
                order: [['id', 'DESC']],
                limit: perPage,
                offset: offset,
            });
            ...
            errLogger(`[APPROVAL_FUND_PACKAGE_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    APPROVAL_FUND_PACKAGE_BONUS_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await ApprovalFundPackageBonuses.findAndCountAll({
                include: {
                    model: User,
                    as: 'from_user',
                    attributes: ['id', 'name', 'phone_number']
    ...
    ALLOCATION_AUTH_PACKAGE = async (req, res) => {
        try {
            ...
            const bankInfo = await AllocationAuthBankInfo.findOne({
                where: { user_id: userId },
                attributes: ['id', 'card_name', 'card_number', 'bank_name'],
                useMaster: true
            });
    ...
    PRIORITY_QUEUEING_PACKAGE = async (req, res) => {
        try {
            const userId = req.user_id;
            let packages = await this.redisHelper.getValue('priority_queueing_packages');
            if (packages) {
                packages = JSON.parse(packages);
            } else {
                packages = await PriorityQueueingPackage.findAll({
                    where: {
                        status: {
                            [Op.ne]: 2
                        }
                ...
                await this.redisHelper.setValue('priority_queueing_packages', JSON.stringify(packages));
            }

            let package_period = await this.redisHelper.getValue('priority_queueing_package_period');
            if (!package_period) {
                const conf = await Config.findOne({ where: { type: 'priority_queueing_package_period' } });
                package_period = conf ? conf.val : '';
                await this.redisHelper.setValue('priority_queueing_package_period', package_period);
            }

            let package_description = await this.redisHelper.getValue('priority_queueing_package_description');
            if (!package_description) {
                const conf = await Config.findOne({ where: { type: 'priority_queueing_package_description' } });
                package_description = conf ? conf.val : '';
                await this.redisHelper.setValue('priority_queueing_package_description', package_description);
            }

            let firstAllocationAuthPackage = await this.redisHelper.getValue(`first_allocation_auth_package_${userId}`);
            if (!firstAllocationAuthPackage) {
                const allocationAuthPackageHistory = await AllocationAuthPackageHistory.findOne({
                    where: {
                        user_id: userId,
                    },
    ...
    BUY_PRIORITY_QUEUEING_PACKAGE = async (req, res) => {

        const lockKey = `lock:buy-priority-queueing-package:${req.ip}`;
        let redisLocked = false;
        const PROCESSING_KEY = `priority_queueing_package_processing_${req.user_id}`
        try {
            let buyOnOff = await this.redisHelper.getValue('buy_product_on_off');
            if (!buyOnOff) {
                const conf = await Config.findOne({ where: { type: 'buy_product_on_off' }, attributes: ['val'] });
            ...
            * =============================== */
            ...
            const aPackage = await PriorityQueueingPackage.findByPk(req.params.id, { useMaster: true });
            if (!aPackage) {
                await this.redisHelper.deleteKey(PROCESSING_KEY);
                return MyResponse(res, this.ResCode.NOT_FOUND.code, false, '产品不存在', {});
            }
            ...
            const userId = req.user_id;
            const history = await PriorityQueueingPackageHistory.findOne({
                where: {
                    user_id: userId,
                    package_id: aPackage.id,
                },
            ...
            try {
                ...
                const pkgHistoryItem = await PriorityQueueingPackageHistory.create({
                    relation: user.relation,
                    user_id: user.id,
                    package_id: aPackage.id,
                    price: aPackage.price,
                    queue_amount: aPackage.queue_amount,
                }, { transaction: t });
                ...
                const relationArr = user.relation.split('/');
                const upLevelIds = (relationArr.slice(1, relationArr.length - 1)).reverse().slice(0, 3);
                commonLogger(`[BUY_PRIORITY_QUEUEING_PACKAGE] Bonus Settings: LV1=${15}%, LV2=${7}%, LV3=${3}%`);
                commonLogger(`[BUY_PRIORITY_QUEUEING_PACKAGE] Uplines: ${upLevelIds.join(',')}`);
                ...
                for (let index = 0; index < upLevelIds.length; index++) {
                    ...
                    commonLogger(`[BUY_PRIORITY_QUEUEING_PACKAGE] Granting bonus ${bonus} to UserID: ${upLevelUser.id}`);

                    cashFlows.push({
                        relation: upLevelUser.relation,
                        user_id: upLevelUser.id,
                        wallet_type: 2,
                        model: 'PriorityQueueingPackageBonuses',
                        type: `下级购买优先排列方案奖励`,
                        amount: bonus,
                        before_amount: Number(upLevelUser.balance),
                        after_amount: Number(upLevelUser.balance) + Number(bonus),
                ...
                if (bonuses.length > 0) {
                    await PriorityQueueingPackageBonuses.bulkCreate(bonuses, { transaction: t });
                    await CashFlow.bulkCreate(cashFlows, { transaction: t });
                }
    ...
    PRIORITY_QUEUEING_PACKAGE_HISTORY = async (req, res) => {
        try {
            const userId = req.user_id;
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            ...
            const { rows, count } = await PriorityQueueingPackageHistory.findAndCountAll({
                include: {
                ...
                attributes: ['id', 'price', 'queue_amount', 'createdAt'],
                order: [['id', 'DESC']],
                limit: perPage,
                offset: offset,
            });
            ...
            errLogger(`[PRIORITY_QUEUEING_PACKAGE_HISTORY][${req.user_id}]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }

    PRIORITY_QUEUEING_PACKAGE_BONUS_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await PriorityQueueingPackageBonuses.findAndCountAll({
                include: {
                    model: User,
                    as: 'from_user',
                    attributes: ['id', 'name', 'phone_number']
