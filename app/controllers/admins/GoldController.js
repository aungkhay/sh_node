...
const { GoldPrice, UserGoldPrice, User, Config, GoldInterest, db, GoldPackageHistory, GoldPackageBonuses, GoldPackageReturn, GoldPackageRepurchase, GoldPlanCheckIn } = require('../../models');
...
class Controller {
    ...
    GOLD_PACKAGE_HISTORY = async (req, res) => {
        try {
            ...
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            const packageId = parseInt(req.query.packageId || 0);
            const phone = req.query.phone;
            const userId = req.user_id;
            const startTime = req.query.startTime;
            const endTime = req.query.endTime;
            ...
            const { rows, count } = await GoldPackageHistory.findAndCountAll({
                include: {
                    model: User,
                    as: 'user',
    ...
    GOLD_PACKAGE_BONUS_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
    ...
    GOLD_PACKAGE_REPURCHASE_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await GoldPackageRepurchase.findAndCountAll({
                include: {
                    model: User,
                    as: 'user',
                    attributes: ['id', 'name', 'phone_number'],
    ...
    GOLD_PLAN_CHECK_IN_HISTORY = async (req, res) => {
        try {
            const page = parseInt(req.query.page || 1);
            const perPage = parseInt(req.query.perPage || 10);
            const offset = this.getOffset(page, perPage);
            ...
            const { rows, count } = await GoldPlanCheckIn.findAndCountAll({
                include: {
                    model: User,
                    as: 'user',
                    attributes: ['id', 'name', 'phone_number'],
