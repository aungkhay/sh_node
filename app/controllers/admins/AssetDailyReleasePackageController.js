...
const { AssetDailyReleasePackage, AssetDailyReleasePackageHistory, User, AssetDailyReleasePackageEarn, AssetDailyReleasePackageBonuses } = require('../../models');
...
class Controller {
    ...
    INDEX = async (req, res) => {
        try {
            const packages = await AssetDailyReleasePackage.findAll({});
    ...
    UPLOAD = async (req, res) => {
        try {
            req.uploadDir = `./uploads/asset_daily_release_packages/`;
            ...
            upload(req, res, async (err) => {
                ...
                const pkg = await AssetDailyReleasePackage.findByPk(req.params.id);
                if (!pkg) {
                    return MyResponse(res, this.ResCode.NOT_FOUND.code, false, '未找到信息', {});
                }

                // Upload to AliOSS
                const dir = 'uploads/asset_daily_release_packages/';
                const fileName = req.file.filename;
                const localFile = path.resolve(__dirname, `../../../uploads/asset_daily_release_packages/${fileName}`);
                const { success } = await this.OSS.PUT(dir, fileName, localFile);
                if (success) {
                    await pkg.update({ cover_image: `/uploads/asset_daily_release_packages/${fileName}` });
            ...
            errLogger(`[AssetDailyReleasePackage][UPLOAD]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    CREATE = async (req, res) => {
        try {
            ...
            await AssetDailyReleasePackage.create(req.body);
            ...
            errLogger(`[AssetDailyReleasePackage][CREATE]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});   
        }
    }
    ...
    UPDATE = async (req, res) => {
        try {
            ...
            errLogger(`[AssetDailyReleasePackage][UPDATE]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    ASSET_DAILY_RELEASE_PACKAGE_HISTORY = async (req, res) => {
        try {
            ...
            const totalBought = await AssetDailyReleasePackageHistory.sum('price', { where: { ...condition, price: { [Op.gt]: 0 } } }) || 0;
            const boughtCount = await AssetDailyReleasePackageHistory.count({ where: { ...condition, price: { [Op.gt]: 0 } } });
            const userBoughtCount = await AssetDailyReleasePackageHistory.count({ where: { ...condition, price: { [Op.gt]: 0 } }, distinct: true, col: 'user_id' });

            const freeBoughtCount = await AssetDailyReleasePackageHistory.count({ where: { ...condition, price: 0 } });
            const freeUserBoughtCount = await AssetDailyReleasePackageHistory.count({ where: { ...condition, price: 0 }, distinct: true, col: 'user_id' });

            const internalUserBought = await AssetDailyReleasePackageHistory.sum('price', {
                where: {
                    ...condition,
            ...
            errLogger(`[AssetDailyReleasePackage][PACKAGE_HISTORY]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
