...
class Controller {
    ...
    INDEX = async (req, res) => {
        try {
            ...
            const { rows, count } = await User.findAndCountAll({
                ...
                where: condition,
                attributes: [
                    'id', 'type', 'name', 'serial_number', 'phone_number', 'invite_code', 'reserve_fund', 'balance', 'sharing_amount', 'total_assets', 'distributed_assets', 
                    'sco_verified_assets', 'approval_fund', 'actual_approval_fund', 'referral_bonus', 'masonic_fund', 'repurchase_fund', 'rank_allowance', 'freeze_allowance',
                    'earn', 'gold', 'gold_interest', 'address', 'address_status', 'agreement_status', 'rank_point', 'level_up_pay', 'win_per_day', 'status', 'political_vetting_status', 
    ...
    CHILD_SUMMARY = async (req, res) => {
        try {
            ...
            const total_deposit_child_count = await Deposit.count({
                where: {
                    status: 1,
                    ... condition
                },
            ...
            const total_withdraw_child_count = await Withdraw.count({
                where: {
                    status: 1,
                    ... condition
                },
            ...
            const data = {
                ...
                total_deposit_child_count: parseInt(total_deposit_child_count || 0),
                total_withdraw_child_count: parseInt(total_withdraw_child_count || 0)
            }
