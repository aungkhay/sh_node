...
const { User, PersonalReservePackage, PersonalReservePackageHistory, db, PersonalReservePackageEarn, PersonalReservePackageBonuses, CashFlow } = require('../../models');
...
class Controller {
    ...
    INDEX = async (req, res) => {
        try {
            const packages = await PersonalReservePackage.findAll();
    ...
    UPLOAD = async (req, res) => {
        try {
            req.uploadDir = `./uploads/personal_reserve_packages/`;
            ...
            upload(req, res, async (err) => {
                ...
                const pkg = await PersonalReservePackage.findByPk(req.params.id);
                if (!pkg) {
                    return MyResponse(res, this.ResCode.NOT_FOUND.code, false, '未找到信息', {});
                }

                // Upload to AliOSS
                const dir = 'uploads/personal_reserve_packages/';
                const fileName = req.file.filename;
                const localFile = path.resolve(__dirname, `../../../uploads/personal_reserve_packages/${fileName}`);
                const { success } = await this.OSS.PUT(dir, fileName, localFile);
                if (success) {
                    await pkg.update({ cover_image: `/uploads/personal_reserve_packages/${fileName}` });
            ...
            errLogger(`[PersonalReservePackage][UPLOAD]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    CREATE = async (req, res) => {
        try {
            ...
            const { product_name, price, period, reserve_earn, release_earn_count, release_earn_count, release_personal_gold_rate, is_release_authorize_letter, purchase_limit, quantity_limit, total_quantity, description, buy_one_get_quantity, can_new_registered_user_get_free } = req.body;
            const newPackage = await PersonalReservePackage.create({
                product_name: product_name,
                price: price,
                period: period,
                reserve_earn: reserve_earn,
                release_earn_count: release_earn_count,
                release_personal_gold_rate: release_personal_gold_rate,
            ...
            errLogger(`[PersonalReservePackage][CREATE]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    UPDATE = async (req, res) => {
        try {
            ...
            errLogger(`[PersonalReservePackage][UPDATE]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    PACKAGE_HISTORY = async (req, res) => {
        try {
            ...
            const { count, rows } = await PersonalReservePackageHistory.findAndCountAll({
                where: condition,
                include: [
                    {
                        model: User,
            ...
            const totalBought = await PersonalReservePackageHistory.sum('price', { where: { ...condition, price: { [Op.gt]: 0 } } }) || 0;
            const boughtCount = await PersonalReservePackageHistory.count({ where: { ...condition, price: { [Op.gt]: 0 } } });
            const userBoughtCount = await PersonalReservePackageHistory.count({ where: { ...condition, price: { [Op.gt]: 0 } }, distinct: true, col: 'user_id' });

            const internalUserBought = await PersonalReservePackageHistory.sum('price', {
                where: {
                    ...condition,
            ...
            errLogger(`[PersonalReservePackage][PACKAGE_HISTORY]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    BONUSES_HISTORY = async (req, res) => {
        try {
            ...
            errLogger(`[PersonalReservePackage][BONUSES_HISTORY]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    EARN_HISTORY = async (req, res) => {
        try {
            ...
            errLogger(`[PersonalReservePackage][EARN_HISTORY]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
}
