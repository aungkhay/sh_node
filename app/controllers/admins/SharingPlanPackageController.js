...
class Controller {
    ...
    INDEX = async (req, res) => {
        try {
            const packages = await SharingPlanPackage.findAll({});
    ...
    UPLOAD = async (req, res) => {
        try {
            req.uploadDir = `./uploads/sharing_plan_packages/`;
            ...
            upload(req, res, async (err) => {
                ...
                const pkg = await SharingPlanPackage.findByPk(req.params.id);
                if (!pkg) {
                    return MyResponse(res, this.ResCode.NOT_FOUND.code, false, '未找到信息', {});
                }

                // Upload to AliOSS
                const dir = 'uploads/sharing_plan_packages/';
            ...
            errLogger(`[SharingPlanPackage][UPLOAD]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    CREATE = async (req, res) => {
        try {
            ...
            await SharingPlanPackage.create(req.body);

            // Log
            await this.adminLogger(req, 'SharingPlanPackage', 'create');
    ...
    UPDATE = async (req, res) => {
        try {
            ...
            // Log
            await this.adminLogger(req, 'SharingPlanPackage', 'update');

            await this.redisHelper.deleteKey(`sharing_plan_packages`); // Clear cache
            return MyResponse(res, this.ResCode.SUCCESS.code, true, '更新成功', {});
        } catch (error) {
            errLogger(`[SharingPlanPackage][UPDATE]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    SHARING_PLAN_PACKAGE_HISTORY = async (req, res) => {
        try {
            ...
            const totalBought = await SharingPlanPackageHistory.sum('price', { where: { ...condition, price: { [Op.gt]: 0 } } }) || 0;
            const boughtCount = await SharingPlanPackageHistory.count({ where: { ...condition, price: { [Op.gt]: 0 } } });
            const userBoughtCount = await SharingPlanPackageHistory.count({ where: { ...condition, price: { [Op.gt]: 0 } }, distinct: true, col: 'user_id' });

            const internalUserBought = await SharingPlanPackageHistory.sum('price', {
                where: {
                    ...condition,
            ...
            errLogger(`[SharingPlanPackage][PACKAGE_HISTORY]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
    ...
    SHARING_PLAN_PACKAGE_EARN_HISTORY = async (req, res) => {
        try {
            ...
            errLogger(`[SharingPlanPackage][EARN_HISTORY]: ${error.stack}`);
            return MyResponse(res, this.ResCode.SERVER_ERROR.code, false, this.ResCode.SERVER_ERROR.msg, {});
        }
    }
}
