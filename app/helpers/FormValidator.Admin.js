...
exports.create_channel = () => {
    return [
        ...
        check('payment_method').not().isEmpty().withMessage('充值方式不能为空')
            ...
            .bail()
            .isIn([1, 2, 3, 4, 5, 6]).withMessage('充值方式必须是 1 到 6 之间'),
...
exports.create_gold_appreciation_package = () => {
    return [
        ...
        check('can_exchange_fragment').optional({ checkFalsy: true })
            .isIn([0, 1])
            .withMessage('是否可以兑换碎片值无效'),
        // status 1-在售, 2-下架, 3-售罄
        check('status').not().isEmpty().withMessage('状态不能为空')
            .bail()
            .isIn([1, 2, 3])
...
exports.create_asset_distribution_package = () => {
    return [
        check('product_name').not().isEmpty().withMessage('产品名称不能为空'),
        check('price').not().isEmpty().withMessage('价格不能为空')
            .bail()
...
exports.create_asset_earn_package = () => {
    return [
        check('product_name').not().isEmpty().withMessage('产品名称不能为空'),
        check('price').not().isEmpty().withMessage('价格不能为空')
            .bail()
...
exports.create_asset_daily_release_package = () => {
    return [
        check('product_name').not().isEmpty().withMessage('产品名称不能为空'),
        check('price').not().isEmpty().withMessage('认购金额不能为空')
            .bail()
            .isNumeric()
...
exports.create_sco_interbank_package = () => {
    return [
        check('product_name').not().isEmpty().withMessage('产品名称不能为空'),
        check('price').not().isEmpty().withMessage('清验资金不能为空')
            .bail()
            .isNumeric()
            .withMessage('清验资金必须是数字'),
        check('period').not().isEmpty().withMessage('周期不能为空')
            .bail()
            .isInt({ min: 1 }).withMessage('周期必须是正整数'),
        check('transfer_out_assets_rate').not().isEmpty().withMessage('转出资产宝余额 (%)不能为空')
            .bail()
            .isNumeric()
            .withMessage('转出资产宝余额 (%)必须是数字'),
        // purchase_limit (NONE,DAILY,TOTAL)
        check('purchase_limit').not().isEmpty().withMessage('购买限制不能为空')
            .bail()
            .isIn(['NONE', 'DAILY', 'TOTAL'])
...
exports.create_approval_fund_package = () => {
    return [
        check('product_name').not().isEmpty().withMessage('产品名称不能为空'),
        check('price').not().isEmpty().withMessage('审批费不能为空')
            .bail()
            .isNumeric()
            .withMessage('审批费必须是数字'),
        check('approval_fund').not().isEmpty().withMessage('审批资金不能为空')
            .bail()
            .isNumeric()
            .withMessage('审批资金必须是数字'),
        check('period').not().isEmpty().withMessage('周期不能为空')
            .bail()
            .isInt({ min: 1 }).withMessage('周期必须是正整数'),
        // purchase_limit (NONE,DAILY,TOTAL)
